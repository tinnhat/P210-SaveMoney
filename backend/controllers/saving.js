...
export const createSaving = async (req, res) => {
  try {
    ...
    if (user.balanced < newSaving.balanced)
      return res.status(500).json({ error: "Số dư không đủ" }); //kiem tra so du tai khoan voi so tien gui
    ...
    let updateUser = await UserModel.findByIdAndUpdate(
      { _id: newSaving.userID },
      { balanced: usernewBalanced },
      { new: true }
    ); //update so du tai khoan
    ...
    const history = {
      ...
      userid: user._id,
      name: user.name,
      detail: `Bạn đã tạo một sổ tiết kiệm online với số tiền ${
        newSaving.balanced
      } với thời hạn ${convertedJSON.duration / 30} tháng`,
    };
...
export const getSavingBySavingID = async (req, res) => {
  try {
    ...
    const date = Math.floor(
      (new Date().getTime() - getsaving.inContract) / 86400000
    ); //tính thời gian đã gữi của sổ milliseconds
    ...
    const termBalanced =
      (getsaving.balanced * finalRate * (getsaving.duration * cycles)) / 360; //lãi kỳ hạn
    const unlimitBalanced =
      getsaving.balanced *
      (((date - getsaving.duration * cycles) * 0.015) / 360); //lãi không kỳ hạn
...
export const withdrawSaving = async (req, res) => {
  try {
    ...
    if (getsaving.status != 1)
      return res.status(500).json({ error: "Sổ đã rút" });
    var interestRate = 0;
    // % lai suat
    switch (getsaving.duration) {
      case 30:
    ...
    console.log("finalRate: ", finalRate);
    const termBalanced =
      (getsaving.balanced * finalRate * (getsaving.duration * cycles)) / 360;
    const unlimitBalanced =
      getsaving.balanced *
      (((date - getsaving.duration * cycles) * 0.015) / 360);
    ...
    const updateSaving = await SavingModel.findByIdAndUpdate(
      getsaving._id,
      {
    ...
    const updateBalanced = await UserModel.findByIdAndUpdate(
      getsaving.userID,
      { balanced: userNewBalanced },
      { new: true }
    );
    ...
    const history = {
      userid: userInfo._id,
      name: userInfo.name,
      detail: `Bạn đã rút sổ tiết kiệm ${convertedJSON.balanced}, ${
        convertedJSON.duration / 30
      } tháng. Bạn đã gữi được ${
        convertedJSON.cycles
      } chu kỳ và tổng số tiền là ${convertedJSON.balancedWithdrawed}`,
    };
    const saveHistory = new HistoryModel(history);
