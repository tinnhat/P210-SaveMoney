...
export const register = async (req, res) => {
  try {
    ...
    if (!username)
      return res.status(400).json({ msg: "Please enter your user name" });
    if (!nationalid)
      return res.status(400).json({ msg: "Please enter your id." });
    if (!gender)
      return res.status(400).json({ msg: "Please choose your gender." });
    if (!dob)
    ...
    if (!passportid)
      return res
        .status(400)
        .json({ msg: "Please enter your your passport ID." });
    if (!dob)
      return res.status(400).json({ msg: "Please enter your date of birth." });
    if (!nationality)
      return res.status(400).json({ msg: "Please enter your nationality." });
    if (!creditcard)
      return res
        .status(400)
        .json({ msg: "Please enter your credit card number." });
    if (!creditcardbrand)
      return res
        .status(400)
        .json({ msg: "Please enter your credit card issuer." });
...
export const deposit = async (req, res) => {
  try {
    ...
    if (typeof inputInfo.moneyDeposit != "number")
      res.status(500).json({ error: "Lỗi Input" });
...
export const withdraw = async (req, res) => {
  try {
    const inputInfo = req.body;
    if (typeof inputInfo.moneyWithdraw != "number")
      res.status(500).json({ error: "Lỗi Input" });
    if (inputInfo.moneyWithdraw < 0)
      res.status(500).json({ error: "Lỗi Input" }); //tránh trường hợp bug số âm
    const userInfo = await UserModel.findById(inputInfo._id);
    const newbalanced = userInfo.balanced - inputInfo.moneyWithdraw;
    if (newbalanced < 0)
      return res.status(500).json({ error: "Số dư không đủ" });
    const userUpdate = await UserModel.findByIdAndUpdate(
      inputInfo._id,
      { balanced: newbalanced },
      { new: true }
